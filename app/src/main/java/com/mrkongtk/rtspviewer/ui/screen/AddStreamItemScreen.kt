...
/**
 * Holds the state for the stream addition form.
 *
...
private data class FieldsValue(
     ...
     * @return A resource ID for the error message if invalid, null otherwise.
     */
    val nameError: Int?
    ...
    /**
     * Converts the current form state into a database entity [RTSPItem].
     */
    val rtspItem: RTSPItem
        get() = RTSPItem(
            id = 0, // ID is 0 as it is auto-generated by the Room database
            name = name,
            uri = uri,
            tags = tags,
            order = -1,
...
/**
 * A Composable screen for adding a new RTSP stream item.
 *
 * @param onSave Callback triggered when the valid form is saved. Passes the created [RTSPItem].
...
fun AddStreamItemScreen(
    ...
    val defaultValue = FieldsValue()

    // State holder for the form input fields
    var fieldsValue by remember {
        mutableStateOf(defaultValue)
    }

    // Main container: Scrollable column to handle small screens or landscape mode
    Column(
        modifier = modifier
            .padding(PaddingM)
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            // Clear Button: Resets the form to default values
            IconButton(
                onClick = {
                ...
                Icon(imageVector = Icons.Default.Clear, stringResource(R.string.clear_button))
            }

            // Save Button: Persists data (enabled only if validation passes)
            IconButton(
                onClick = {
                    onSave(fieldsValue.rtspItem)
                },
